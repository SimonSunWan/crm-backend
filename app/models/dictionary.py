...
class DictionaryType(Base, TimestampMixin):
    ...
    name = Column(
        String, unique=True, index=True, nullable=False, comment="字典类型名称"
    )
    code = Column(
        String, unique=True, index=True, nullable=False, comment="字典类型编码"
    )
    description = Column(String, nullable=True, comment="字典类型描述")

    # 关联字典枚举
    enums = relationship(
        "DictionaryEnum", back_populates="type", cascade="all, delete-orphan"
    )
