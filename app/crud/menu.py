...
class CRUDMenu(CRUDBase[Menu]):
    ...
    def get_by_name_and_parent(self, db: Session, name: str, parent_id: Optional[int] = None) -> Optional[Menu]:
    ...
    def get_by_auth_mark_and_parent(self, db: Session, auth_mark: str, parent_id: Optional[int] = None) -> Optional[Menu]:
        """根据权限标识和父ID获取菜单（同层级查重）"""
        return db.query(self.model).filter(
            self.model.auth_mark == auth_mark,
            self.model.parent_id == parent_id
        ).first()
    ...
    def update_parent_enable_status(self, db: Session, menu_id: int):
        ...
        children_menus = db.query(self.model).filter(
            self.model.parent_id == parent_id,
            self.model.menu_type == "menu"
        ).all()
