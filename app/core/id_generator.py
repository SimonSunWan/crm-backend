...
class OrderIDGenerator:
    ...
    def __init__(self, strategy: IDGenerationStrategy = IDGenerationStrategy.DATABASE_LOCK):
    ...
    def _generate_with_database_lock(self, db: Session, prefix: str, max_retries: int) -> str:
        """使用数据库锁策略生成ID"""
            ...
            try:
                ...
                with db.begin_nested():
                    ...
                    existing_order = db.execute(
                        text(f"SELECT id FROM {prefix.lower()}_orders WHERE id = :order_id"),
                        {"order_id": order_id}
                    ).fetchone()
