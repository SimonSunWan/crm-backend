...
def validate_system_code(db: Session, system_code: str, setting_key: str = "REGISTER_SYSTEM_CODE") -> bool:
    """
    验证系统码
...
def validate_email_uniqueness(db: Session, email: str, user_crud, exclude_user_id: int = None) -> bool:
    """
    验证邮箱唯一性
...
def validate_username_uniqueness(db: Session, username: str, user_crud, exclude_user_id: int = None) -> bool:
    """
    验证用户名唯一性
...
def validate_role_name_uniqueness(db: Session, role_name: str, role_crud, exclude_role_id: int = None) -> bool:
    """
    验证角色名称唯一性
...
def validate_role_code_uniqueness(db: Session, role_code: str, role_crud, exclude_role_id: int = None) -> bool:
    """
    验证角色编码唯一性
...
def validate_menu_name_uniqueness(db: Session, menu_name: str, parent_id: int, menu_crud, exclude_menu_id: int = None) -> bool:
    """
    验证菜单名称唯一性（同层级）
    ...
    if existing_menu and (exclude_menu_id is None or existing_menu.id != exclude_menu_id):
        raise HTTPException(status_code=400, detail="菜单名称已存在")
    return True


def validate_menu_path_uniqueness(db: Session, menu_path: str, menu_crud, exclude_menu_id: int = None) -> bool:
    """
    验证菜单路径唯一性
...
def validate_menu_auth_mark_uniqueness(db: Session, auth_mark: str, parent_id: int, menu_crud, exclude_menu_id: int = None) -> bool:
    """
    验证菜单权限标识唯一性（同层级）
